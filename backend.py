...
class TransportGraph:
    ...
    def dijkstra(self, start_id: int, end_id: int,
        ...
        while curr is not None and prev[curr] is not None:
            ...
            path.append((curr, dist[curr], edge_data))
            curr = prev_node
...
class PassTranquilleAPI:
    ...
    def initialize(self):
        self.db.connect()
        self.graph = TransportGraph(self.db)
        try:
            model, scaler = joblib.load('traffic_model.pkl')
    ...
    def get_routes(self, from_lon: float, from_lat: float,
                   to_lon: float, to_lat: float,
                   departure_time: Optional[datetime] = None,
                   is_joj_event: bool = False) -> Dict:
        if departure_time is None:
            departure_time = datetime.now()

        journeys = self.graph.journey_with_walk(
            from_lon, from_lat, to_lon, to_lat, departure_time
        )

        if not journeys:
            return {
                'status': 'error',
                'message': 'Aucun itineraire trouve pour cette destination'
            }

        results = []
        for journey in journeys:
            predicted_duration = self.predictor.predict_journey_duration(
                journey, departure_time, is_joj_event
            )
            journey.score = predicted_duration
            segments = self._decompose_journey(journey)

            results.append({
                'rank': len(results) + 1,
                'total_duration_seconds': journey.total_duration,
                'predicted_duration_seconds': predicted_duration,
                'total_duration_formatted': self._format_duration(journey.total_duration),
                'predicted_duration_formatted': self._format_duration(predicted_duration),
                'total_distance_meters': journey.total_distance,
                'total_transfers': journey.total_transfers,
                'congestion_level': self.predictor.get_congestion_level(
                    departure_time.hour, departure_time.weekday()
                ),
                'segments': segments,
                'steps': [
                    {
                        'from': {'name': s.from_stop.name, 'lat': s.from_stop.latitude, 'lon': s.from_stop.longitude},
                        'to': {'name': s.to_stop.name, 'lat': s.to_stop.latitude, 'lon': s.to_stop.longitude},
                        'mode': s.mode,
                        'mode_name': MODE_NAMES.get(s.mode, s.mode),
                        'color': MODE_COLORS.get(s.mode, '#999'),
                        'distance': s.distance_meters,
                        'distance_formatted': s.distance_formatted,
        ...
        results.sort(key=lambda x: x['predicted_duration_seconds'])
        ...
        return {
            ...
            'is_joj_event': is_joj_event,
            'routes': results[:2]
        }

    def _decompose_journey(self, journey: Journey) -> List[Dict]:
        if not journey.steps:
            return []
        segments = []
        current_mode = journey.steps[0].mode
        current_stops = [journey.steps[0].from_stop]
        current_distance = 0
        current_duration = 0

        for step in journey.steps:
            if step.mode == current_mode:
                current_stops.append(step.to_stop)
                current_distance += step.distance_meters
                current_duration += step.duration_seconds
            else:
                segments.append({
                    'mode': current_mode,
                    'mode_name': MODE_NAMES.get(current_mode, current_mode),
                    'color': MODE_COLORS.get(current_mode, '#999'),
                    'from_stop': current_stops[0].name,
                    'to_stop': current_stops[-1].name,
                    'stops_count': len(current_stops),
                    'distance_meters': round(current_distance, 0),
                    'distance_formatted': f"{current_distance/1000:.1f} km" if current_distance >= 1000 else f"{current_distance:.0f} m",
                    'duration_seconds': round(current_duration, 0),
                    'duration_formatted': self._format_duration(current_duration),
                    'coordinates': [(s.longitude, s.latitude) for s in current_stops]
                })
                current_mode = step.mode
                current_stops = [step.from_stop, step.to_stop]
                current_distance = step.distance_meters
                current_duration = step.duration_seconds
    ...
    def get_all_stops(self) -> List[Dict]:
        query = "SELECT id, name, longitude, latitude, transport_type, zone FROM stops ORDER BY transport_type, name"
        return self.db.execute(query)

    def get_stops_by_zone(self, zone: str) -> List[Dict]:
        query = "SELECT * FROM stops WHERE zone ILIKE %s ORDER BY name"
        return self.db.execute(query, (f'%{zone}%',))
...
if __name__ == '__main__':
    api = PassTranquilleAPI()
    api.initialize()
    print("\n" + "="*60)
    print("TEST: Gare de Dakar -> Prefecture de Guediawaye")
    print("="*60)
    result = api.get_routes(
        from_lon=-17.43400587808597,
        from_lat=14.676798251219608,
        to_lon=-17.38714236721617,
        to_lat=14.772063808481434,
        departure_time=datetime(2026, 6, 15, 8, 30),
        is_joj_event=True
    )
    if result['status'] == 'success':
        for route in result['routes']:
            print(f"\nItineraire #{route['rank']} - {route['predicted_duration_formatted']}")
            print(f"   Distance: {route['total_distance_meters']/1000:.1f} km | Transferts: {route['total_transfers']}")
            for seg in route['segments']:
                print(f"   - {seg['mode_name']}: {seg['from_stop']} -> {seg['to_stop']} ({seg['distance_formatted']}, {seg['duration_formatted']})")
    api.close()